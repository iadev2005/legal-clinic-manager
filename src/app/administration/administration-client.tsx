...
export default function Administration({ currentUser }: { currentUser: any }) {
  ...
  const [activeTab, setActiveTab] = useState<
    "users" | "centers" | "semestres" | "materias"
  >("users");
  ...
  const handleEdit = async (item: any, type: string = activeTab) => {
    ...
    if (type === "users") {
      ...
      if (participacionesRes.success) {
        setStudents(
          participacionesRes.data.filter(
            (p: any) =>
              p.tipo === "Voluntario" ||
              p.tipo === "Inscrito" ||
              p.tipo === "Egresado",
  ...
  const confirmToggleStatus = async () => {
    ...
    if (errorCount > 0) {
      if (successCount > 0) {
        alert(
          `Se procesaron ${successCount} usuarios. Hubo ${errorCount} errores (posiblemente por permisos).`,
        );
      } else {
        alert(
          `No se pudieron procesar los usuarios seleccionados (posiblemente por permisos).`,
        );
      }
    }
  ...
  const ManagementUserColumns: Column<(typeof users)[0]>[] = [
    {
      header: "ID",
      accessorKey: "id",
      className: "font-bold px-2 py-2 text-xs leading-tight",
    },
    {
      header: "Usuario",
      accessorKey: "user",
      className: "font-bold pl-2 py-2 text-sm",
    },
    {
      header: "Rol",
      accessorKey: "role",
      ...
      render: (row) => {
        ...
        return (
          <div className="flex justify-center">
            <span
              className={`px-2 py-0.5 rounded-full font-bold text-xs ${badgeClass}`}
            >
              {role}
            </span>
          </div>
      ...
      render: (row) => {
        ...
        return (
          <div className="flex gap-2 justify-center">
            ...
            >
              <span
                className={`icon-[mdi--swap-horizontal] text-3xl ${isDisabled ? "text-gray-400" : "text-red-600"} group-hover:scale-110 transition-transform`}
              ></span>
            </button>
          </div>
        );
  ...
  const SemestresColumns: Column<any>[] = [
    {
      header: "Término",
      accessorKey: "term",
      className: "font-bold px-2 py-2 text-sm",
    },
    {
      header: "Fecha Inicio",
    ...
    switch (tab) {
      case "users":
        ...
        if (filtroRol) {
          filtered = filtered.filter(
            (item) => item.role?.toLowerCase() === filtroRol.toLowerCase(),
          );
        }
        // Filtro por estatus
        if (filtroEstatus) {
      ...
      case "centers":
        ...
        if (filtroEstado) {
          const estadoId = parseInt(filtroEstado);
          if (!isNaN(estadoId)) {
            filtered = filtered.filter((item) => item.id_estado === estadoId);
          }
        }
        ...
        if (filtroMunicipio) {
          ...
          if (!isNaN(municipioId)) {
            filtered = filtered.filter(
              (item) => item.id_municipio === municipioId,
            );
          }
        }
        break;
    ...
    <div className="w-full h-screen min-h-screen bg-neutral-50 flex flex-col justify-start items-center overflow-hidden">
      <div className="w-full h-full p-6 flex flex-col overflow-hidden">
        <div className="self-stretch flex flex-col justify-start items-start flex-none">
          <h1 className="self-stretch justify-start text-sky-950 text-4xl font-bold tracking-tight">
            Administración
          </h1>
          <h1 className="self-stretch justify-start text-[#325B84] text-lg font-medium">
            Gestiona las cuentas de los usuarios, roles y las maestras del
            sistemas.{" "}
          </h1>
        </div>
            ...
            <div className="flex flex-wrap items-center gap-4 w-full">
              ...
              {activeTab !== "materias" && (
                <button
                  ...
                  className={`px-4 py-2.5 rounded-xl font-semibold transition-all duration-300 flex items-center gap-2 cursor-pointer border-2 ${
                    isFilterPanelOpen
                ...
                >
                  ...
                  <span
                    className={`icon-[mdi--chevron-down] text-lg transition-transform duration-300 ${
                ...
                {selectedItems.length > 0 ? (
                  ...
                  >
                    <span
                      className={
                        activeTab === "users"
                          ? "icon-[mdi--account-convert-outline] text-xl"
                          : "icon-[mdi--trash-can-outline] text-xl"
                      }
                    ></span>
                    {activeTab === "users"
                      ? `Cambiar Estatus (${selectedItems.length})`
                  ...
                  <>
                    ...
                    <button
                      ...
                      className="bg-sky-950 text-white px-4 py-2.5 rounded-xl font-semibold hover:bg-[#325B84] transition-colors cursor-pointer"
                    >
                      Agregar
                    </button>
                  </>
          ...
          {activeTab !== "materias" && (
            <div className="flex justify-between items-center mb-2 px-2">
              <div className="text-sm text-sky-950 font-medium">
                Mostrando {paginatedData.length} de {fullFilteredData.length}{" "}
                registros
              </div>
            </div>
          )}
...
function MateriasHierarchy({
  ...
  const getCategorias = (materiaId: string) =>
    categorias.filter((c: any) => String(c.materiaid) === String(materiaId));
  // Use flexible matching for subcategory parent ID (categorymateriaid or categorylegalfieldid)
  const getSub = (catId: string) =>
    subcategorias.filter(
      (s: any) =>
        String(s.categorymateriaid) === String(catId) ||
        String(s.categorylegalfieldid) === String(catId),
    );
  const getAmbitos = (subId: string) =>
    ambitos.filter(
      (a: any) => String(a.categorylegalfieldid) === String(subId),
    );
    ...
    <div className="h-full overflow-y-auto p-4 space-y-6 pb-20">
      ...
      {materias.map((materia: any) => {
        ...
        return (
          <div
            key={materia.id}
            className="border border-sky-100 rounded-xl overflow-hidden bg-white shadow-md transition-shadow hover:shadow-lg"
          >
            {/* Materia Header */}
            <div
            ...
            >
              <div className="flex items-center gap-4">
                <span
                  className={`icon-[mdi--chevron-down] text-2xl text-sky-800 transition-transform duration-300 ${isExpanded ? "rotate-180" : ""}`}
                ></span>
                <span className="icon-[mdi--book-open-page-variant] text-2xl text-sky-700"></span>
                <span className="font-bold text-sky-950 text-xl">
                  {materia.nombre}
                </span>
              </div>
              <div
                className="flex items-center gap-3 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity"
                onClick={(e) => e.stopPropagation()}
              >
                <button
                  onClick={() =>
                    onCreate("categorias", { materiaId: materia.id })
                  }
                ...
                <button
                  onClick={() => onEdit(materia, "materias")}
                  className="p-2 hover:bg-sky-200 rounded-full text-sky-700 transition-colors cursor-pointer"
                  title="Editar - Materia"
                >
                  <span className="icon-[uil--pen] text-2xl"></span>
                </button>
                <button
                  onClick={() => onDelete(materia, "materias")}
                  className="p-2 hover:bg-red-200 rounded-full text-red-600 transition-colors cursor-pointer"
              ...
              <div className="p-6 space-y-6 bg-white animate-in slide-in-from-top-2 duration-300">
                {getCategorias(materia.id).map((categoria: any) => {
                  ...
                  return (
                    <div
                      key={categoria.id}
                      className="pl-6 border-l-4 border-sky-200 ml-2 group/cat"
                    >
                      <div
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <span
                            className={`icon-[mdi--chevron-down] text-xl text-sky-600 transition-transform duration-300 ${isCatExpanded ? "rotate-180" : ""}`}
                          ></span>
                        ...
                        <div
                          className="flex items-center gap-2 opacity-100 sm:opacity-0 sm:group-hover/cat:opacity-100 transition-opacity"
                          onClick={(e) => e.stopPropagation()}
                        >
                          <button
                            onClick={() =>
                              onCreate("subcategorias", {
                                categorylegalfieldid: categoria.id,
                              })
                            }
                          ...
                          <button
                            onClick={() => onEdit(categoria, "categorias")}
                            className="p-1.5 hover:bg-sky-100 rounded-full text-sky-700 transition-colors cursor-pointer"
                            title="Editar - Categoría"
                          >
                            <span className="icon-[uil--pen] text-xl"></span>
                          </button>
                          <button
                            onClick={() => onDelete(categoria, "categorias")}
                            className="p-1.5 hover:bg-red-100 rounded-full text-red-600 transition-colors cursor-pointer"
                        ...
                        <div className="pl-8 space-y-4 animate-in slide-in-from-top-1 duration-200">
                          {getSub(categoria.id).map((sub: any) => {
                            ...
                            return (
                              <div
                                key={sub.id}
                                className="bg-gray-50 rounded-xl p-4 border border-gray-100 shadow-sm group/sub transition-all hover:border-gray-300 hover:shadow-md"
                              >
                                <div
                                ...
                                >
                                  <div className="flex items-center gap-3 flex-1">
                                    <span
                                      className={`icon-[mdi--chevron-down] text-lg text-gray-500 transition-transform duration-300 ${isSubExpanded ? "rotate-180" : ""}`}
                                    ></span>
                                  ...
                                  <div
                                    className="flex items-center gap-2 opacity-100 sm:opacity-0 sm:group-hover/sub:opacity-100 transition-opacity"
                                    onClick={(e) => e.stopPropagation()}
                                  >
                                    <button
                                      onClick={() =>
                                        onCreate("ambitos", { longid: sub.id })
                                      }
                                    ...
                                    <button
                                      onClick={() =>
                                        onEdit(sub, "subcategorias")
                                      }
                                      className="p-1.5 hover:bg-gray-200 rounded-full text-gray-600 transition-colors cursor-pointer"
                                    ...
                                    <button
                                      onClick={() =>
                                        onDelete(sub, "subcategorias")
                                      }
                                      className="p-1.5 hover:bg-red-100 rounded-full text-red-500 transition-colors cursor-pointer"
                                ...
                                {/* Ambitos List */}
                                {isSubExpanded && (
                                  <div className="pl-10 pt-3 grid grid-cols-1 gap-3 animate-in fade-in duration-300">
                                    {getAmbitos(sub.id).map((ambito: any) => (
                                      <div
                                        key={ambito.id}
                                        className="flex justify-between items-start bg-white border border-gray-200 rounded-lg px-4 py-3 text-base shadow-sm group/amb hover:border-blue-200 hover:shadow-md transition-all"
                                      >
                                        <div className="flex-1">
                                        ...
                                        <div className="flex gap-1 opacity-100 sm:opacity-0 sm:group-hover/amb:opacity-100 transition-opacity">
                                          <button
                                            onClick={() =>
                                              onEdit(ambito, "ambitos")
                                            }
                                            className="p-1.5 hover:bg-blue-50 rounded-full text-blue-600 transition-colors cursor-pointer"
                                            title="Editar - Ámbito Legal"
                                          >
                                            <span className="icon-[uil--pen] text-base"></span>
                                          </button>
                                          <button
                                            onClick={() =>
                                              onDelete(ambito, "ambitos")
                                            }
                                            className="p-1.5 hover:bg-red-50 rounded-full text-red-400 transition-colors cursor-pointer"
                                            title="Eliminar - Ámbito Legal"
                                          >
                                    ...
                                    {getAmbitos(sub.id).length === 0 && (
                                      <span className="text-sm text-gray-400 italic pl-1 py-1">
                                        Sin ámbitos legales
                                      </span>
                                    )}
                                  </div>
                          ...
                          {getSub(categoria.id).length === 0 && (
                            <div className="text-base text-gray-400 italic py-2">
                              Sin subcategorías
                            </div>
                          )}
                        </div>
                ...
                {getCategorias(materia.id).length === 0 && (
                  <div className="text-base text-gray-400 italic pl-6 py-2">
                    Sin categorías
                  </div>
                )}
              </div>
